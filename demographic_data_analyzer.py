...
def calculate_demographic_data(print_data=True):
    ...
    # Group by country and salary, then count
    country_salary_counts = df.groupby(['native-country', 'salary']).size().unstack(fill_value=0)

    # Calculate percentage earning >50K for each country
    country_salary_percentage = country_salary_counts['>50K'] / (country_salary_counts['>50K'] + country_salary_counts['<=50K']) * 100
    country_salary_percentage = round(country_salary_percentage,1)
